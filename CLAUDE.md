...
├── tests/              <- 1041 tests, estructura espejo de src
├── scripts/
...
| S12 | Cierre del prototipo | EN CURSO — TRACKDBG promovido a diagnóstico permanente (marcador `[REVERT]` removido; logs DEBUG `TRACKDBG …` + runbook `tracker_tuning.md` intactos). Hardening final: audit confirmó código ya sólido + sandboxing systemd **verificado en hardware** (exposure 4.4 OK, probing WiFi/BLE/MQTT intactos; `ProtectClock` removido pre-deploy — su `DeviceAllow` implícito bloqueaba cámaras/Hailo/GPIO). Revisión cross-proyecto multi-agente (2026-06-09): 19 hallazgos fixeados en `72a6497` (ALTA: drain completo del outbox MQTT + cap de body del viewer; MEDIA: salt del dedup recargado en proceso vivo, mantenimiento del main loop al tope del loop + escalación de watchdog, BLE stop-event por generación, sesiones admin random+expiry, persist_event descarta constraint violations, shadow persist atómico, LED de fallo de init cableado, best_frame con crops, regla 3 stitching unidireccional, tracker pass-2 consumidas + sentinel z=0; y 7 BAJAS). Perf CPU (`e3d894f`+`7b7c62c`): 162%→102% (RGB888 sin cvtColor — nombres de formato picamera2 invertidos, rect_r lazy, pre-filtro de bytes en scapy: BPF kernel NO compila en DLT nexmon) →**~50% en escena vacía** con `vision.idle_throttle` (10 FPS idle, wake instantáneo), **validado E2E en piloto** (9 cruces contados y persistidos en RDS bajo throttle, alturas y conf plausibles). Status LED simplificado a 6 estados (3 colores puros × fijo/blink): se eliminó el amarillo (R+G se veía verdoso por balance de brillo) y el estado `BOOT_FAILURE` se fundió en `HARDWARE_FAULT` (rojo fijo) — el LED no puede señalizar un fallo previo a su propio init, y la acción del operador es la misma; señales internas `boot_*`→`init_*`. S10 cerrado (regla de alerta `death_emit/total > 0.3` + 12 reglas en 3 grupos). Coverage report (`docs/coverage_report.md`, 82% total; núcleo 88–98%) + pasada de hardening de tests: guards del death-emit aislados (counter 97%) + resiliencia de las Lambdas (persist_event/ingest_pos 98%) + fix de race latente en el test del viewer. Gantt regenerado (~180h efectivas). **1039 tests verde (+2 skipped)**. Pendiente: entregables documentales del TFG (fuera del repo). |
| Ext. | Rollout de flota (post-PoC, fuera del plan original) | PENDIENTE — OTA con AWS IoT Jobs + S3 (firma Ed25519, swap atómico A/B + rollback automático, canario, ventana de horario cerrado). Diseño en `docs/ota_design.md` (7 fases, ~45-73h). Junto con Multi-AZ + Route53 + Managed Grafana del backlog. |
...
- `vision.idle_throttle` — FPS adaptativo en escena vacía (default **OFF** fleet-wide; validado E2E en piloto 2026-06-09, ON ahí). Sin actividad sostenida `idle_after_seconds` (2s), el loop baja a `idle_fps` (10) y TODA la cadena baja con él (el productor de captura está backpressured al consumidor): CPU ~102%→~50%. Busy conservador: track no-estacionario (un track nuevo NUNCA clasifica estacionario), `ghost_count > 0` (a FPS bajo el IoU de adopción cae), detecciones > tracks estacionarios, o preview con subscribers. **Gotcha**: el `fps` de telemetría muestra ~10 en horas vacías — es la señal del throttle trabajando, NO una falla; no crear alertas de FPS bajo sin excluir este estado. El sleep va después del registro de latencia (no infla `frame_latencies_ms`).
...
- **Interpretación reporte calibración**: RMS estéreo <0.5px es necesario pero no suficiente. **El verdict depende del ground-truth en centro** (cinta/láser). Baseline estimada debe caer ±1-2mm del diseño 140mm con cobertura diversa (barrido o las 20 poses guiadas); ratio borde/centro es informativo, no gate.
