...
| S12 | Cierre del prototipo | EN CURSO — TRACKDBG promovido a diagnóstico permanente (marcador `[REVERT]` removido; logs DEBUG `TRACKDBG …` + runbook `tracker_tuning.md` intactos). Hardening final: audit confirmó código ya sólido (cero TODO/FIXME, subprocess con timeout, excepciones angulares, watchdog systemd OK) + capa extra de sandboxing systemd (`systemd-analyze security`) agregada al `.service` — **pendiente smoke-test en Pi** (ver checklist inline). Pendiente: entregables. |
