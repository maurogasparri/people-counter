...
| `min_count_height_m` | 1.0 | Filtro anti-FP no-humanos por altura mediana; 1.0 (el valor evaluado, ahora default del template) filtra perros/objetos <1m sin perder personas de más de 1 m. 0.0 lo apaga. Un track sin altura de SGBM no lo filtra ESTE guard, pero sí lo alcanza `min_count_confidence`: el reparto es altura medida bajo el umbral → `min_count_height_m`; sin altura y confianza baja → `min_count_confidence`. La limitación L5 de `docs/benchmark_results.md` describe el punto ciego que queda: cerca del umbral, margen y error de medición son del mismo orden. |
