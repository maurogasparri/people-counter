...
- `--low-light`: preset para cuarto chico / luz difícil — afloja los gates de captura de `assess_frame_quality` (exposure, blur, corner-sharp, L/R balance). **No altera el algoritmo de calibración**: lo que retira son las garantías automáticas sobre la calidad de las capturas, de modo que la carga pasa a la verificación explícita. Una calibración obtenida así **no debe darse por buena sin verificar** — RMS de reproyección, control contra distancia medida y, en última instancia, la exactitud de estatura que mide TC-08. La calibración desplegada en este prototipo se obtuvo con esta bandera y superó esa verificación (−2,69 % en el centro a 1.765 mm, 100 % de relleno); ver [`docs/benchmark_results.md`](docs/benchmark_results.md) §3
...
del prototipo (RMS de reproyección estéreo 0,290 px) y el más práctico en espacios chicos / luz
