...
Reproducible: py validation/demo_crossings.py
"""
...
AFIRMA = {
    ...
    "conf_min": 0.816, "conf_max": 0.898,
    "lat_mediana": 0.194, "lat_max": 0.225,
}
...
def main() -> int:
    ...
    chequeos = [
        ...
        ("confianza mínima", f"{min(confs):.4f}", AFIRMA["conf_min"],
         round(min(confs), 3) == AFIRMA["conf_min"]),
        ("confianza máxima", f"{max(confs):.4f}", AFIRMA["conf_max"],
         round(max(confs), 3) == AFIRMA["conf_max"]),
